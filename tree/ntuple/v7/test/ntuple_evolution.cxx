...
struct AddedIntermediateDerived : public AddedIntermediateBase {
   int fDerived = 3;
      ...
      void *ptr = writer->GetModel().GetDefaultEntry().GetPtr<void>("f").get();
      DeclarePointer("AddedIntermediateDerived", "ptrAddedIntermediateDerived", ptr);
      ProcessLine("ptrAddedIntermediateDerived->fBase = 71;");
      ProcessLine("ptrAddedIntermediateDerived->fDerived = 93;");
      writer->Fill();
...
struct AddedIntermediateDerived : public AddedIntermediate {
   ...
   reader->LoadEntry(0);
   EXPECT_EVALUATE_EQ("ptrAddedIntermediateDerived->fBase", 1);
   EXPECT_EVALUATE_EQ("ptrAddedIntermediateDerived->fIntermediate", 2);
   EXPECT_EVALUATE_EQ("ptrAddedIntermediateDerived->fDerived", 3);

   reader->LoadEntry(1);
   EXPECT_EVALUATE_EQ("ptrAddedIntermediateDerived->fBase", 71);
   EXPECT_EVALUATE_EQ("ptrAddedIntermediateDerived->fIntermediate", 2);
   EXPECT_EVALUATE_EQ("ptrAddedIntermediateDerived->fDerived", 93);
}
