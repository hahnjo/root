...
private:
  Error applyFixup(LinkGraph &G, Block &B, const Edge &E) const {
    ...
    case R_RISCV_JAL: {
      int64_t Value = E.getTarget().getAddress() + E.getAddend() - FixupAddress;
      uint32_t Imm20 = extractBits(Value, 20, 20) << 31;
      uint32_t Imm10_1 = extractBits(Value, 10, 1) << 21;
      uint32_t Imm11 = extractBits(Value, 11, 11) << 20;
      uint32_t Imm19_12 = extractBits(Value, 19, 12) << 12;
      uint32_t RawInstr = *(little32_t *)FixupPtr;
    ...
    case R_RISCV_ADD8: {
      int64_t Value =
           ...
           *(reinterpret_cast<const uint8_t *>(FixupAddress)) +
           E.getAddend());
    ...
    case R_RISCV_SUB6: {
      int64_t Value =
          *(reinterpret_cast<const uint8_t *>(FixupAddress)) &
          0x3f - E.getTarget().getAddress() - E.getAddend();
